%define rubyxver    <%= rubyxver   %>
%define rubyver     <%= rubyver    %>
%define _patchlevel <%= patchlevel %>
...
%define ruby_tk_git_revision  415a3ef9ab82c65a7abc
...
# patches brought over from ruby 1.8.6
...
LD_LIBRARY_PATH=$RPM_BUILD_ROOT%{_libdir} RUBYLIB=$RPM_BUILD_ROOT%{vendorarchbase}/%{rubyxver}:$RPM_BUILD_ROOT%{vendorarchbase}/%{rubyxver}/%{_normalized_cpu}-%{_target_os} make -C $rubybuilddir DESTDIR=$RPM_BUILD_ROOT install-doc

mkdir -p $RPM_BUILD_ROOT%{sitelibbase}/%{rubyxver}
mkdir -p $RPM_BUILD_ROOT%{sitearchbase}/%{rubyxver}/%{_normalized_cpu}-%{_target_os}

# remove shebang
for i in $RPM_BUILD_ROOT%{vendorlibbase}/1.8/{abbrev,generator,irb/{cmd/subirb,ext/save-history},matrix,rdoc/{markup/sample/rdoc2latex,parsers/parse_rb},set,tsort}.rb; do
 sed -i -e '/^#!.*/,1D' $i
done
